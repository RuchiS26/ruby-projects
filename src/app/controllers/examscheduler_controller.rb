...
class ExamschedulerController < ApplicationController
  ...
  def parseSchedule
    ...
    (1..@student.length - 1).each do |i|
      u = User.find_by_uid(@student[i][0])
      next if u

      u = User.create(uid: @student[i][0], email: @student[i][0] + '@ait.asia', password: 'dragon',
    ...
    (1..@student_course.length - 1).each do |i|
      sc = Register.create(course: Course.find_by_course_code(@student_course[i][3]),
                           user: User.find_by_uid(@student_course[i][0]))
    end
  ...
  def check_admin
    ...
    redirect_to request.referrer || examscheduler_admin_path, alert: 'Admins only!'
  end
